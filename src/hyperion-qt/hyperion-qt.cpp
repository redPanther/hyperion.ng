...
int main(int argc, char ** argv)
{
	...
	try
	{
		...
		QtGrabber grabber(parser.isSet(argDisplay), argWidth.getInt(parser), argHeight.getInt(parser), 1000 / argFps.getInt(parser));
		...
		else
		{
			...
			QObject::connect(&grabber, SIGNAL(sig_screenshot(const Image<ColorRgb> &)), &protoWrapper, SLOT(receiveImage(Image<ColorRgb>)));
